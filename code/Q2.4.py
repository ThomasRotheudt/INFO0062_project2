...
for sample_size in sample_sizes:
    ...
    if KNN:
        k = 5
        knn_model_fn = KNeighborsRegressor(n_neighbors=k)
    ...
    if LASSO:
        alpha = 0.01
        lasso_model_fn = Lasso(alpha=alpha, random_state=SEED)
    ...
    if TREE:
        max_depth = 5
        tree_model_fn_fixed = RegressionTree(max_depth=max_depth, random_state=SEED)
        ...
        tree_model_fn_full = RegressionTree(random_state=SEED)
